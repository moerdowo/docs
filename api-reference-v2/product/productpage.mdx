...
openapi: "GET /endpointproductv2/productpage"
description: "Endpoint where you can get your products"
---

<Note>
  **Need the total number of products?** This endpoint only returns the product
  list and does **not** include a `total`. For aggregated statistics — product
  count, customer count, transaction count, and TPV — use the
  **[statistics endpoint](/api-reference-v2/statistics/statistics)**
  (`GET /v2/statistics`) instead.
</Note>
...
```Sandbox Sandbox
https://api.mayar.io/hl/v2/products?limit=10
```
...
<ParamField query="type" path="type" type="string">
  Filter products by product type (e.g. `generic_link`, `event`, `webinar`,
  `digital_product`).
</ParamField>
