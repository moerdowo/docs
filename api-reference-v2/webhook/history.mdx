...
<Note>
  This `/history` endpoint only returns **old** webhook history. For the current
  and up-to-date webhook history, please use the new endpoint:
  `GET /hl/v2/webhooks/new-history` (see
  [Webhook New History](/api-reference-v2/webhook/newhistory)).
</Note>
...
```Sandbox Sandbox
https://api.mayar.io/hl/v2/webhooks/history?limit=10
```
