...
```Sandbox Sandbox
https://api.mayar.io/hl/v2/memberships/members?productId=(productId)
```
...
Each member row carries the membership-customer fields plus the related tier and
customer, exposed in **both** forms:

- nested objects (`customer`, `membershipTier`) — the shape used by every other
  endpoint in this app, and
- the flattened dotted-key columns (`customer.email`,
  `membershipTier.name`, ...) kept for backward compatibility.
...
<ResponseField name="membershipTier" type="object" >
  Nested tier object: `{ "name": "...", "gracePeriodInDays": "..." }`.
</ResponseField>

<ResponseField name="customer" type="object" >
  Nested customer object: `{ "name": "...", "mobile": "...", "email": "..." }`.
</ResponseField>
