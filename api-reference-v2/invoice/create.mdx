...
<ParamField path="items[].rate" type="number" required>
  Price per item. Negative values are accepted and can be used as a discount line
  (e.g. `{ "description": "Diskon", "quantity": 1, "rate": -10000 }`). The
  invoice total must remain greater than zero.
</ParamField>
...
<ResponseField name="400 Invoice total must be greater than zero" type="object">
  Returned when the sum of `rate × quantity` (plus tax) is not positive, e.g. a
  discount line larger than the subtotal:
  `{ "statusCode": 400, "messages": "Invoice total must be greater than zero" }`.
</ResponseField>
