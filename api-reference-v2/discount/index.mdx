...
{/* Notes for maintainers: the response example below is REPRESENTATIVE — it is
    transcribed from the supplied source specification for this list endpoint,
    not captured live. The list projection deliberately differs from the
    single-record projection documented on detail.mdx: items are returned as a
    flat `coupons` array carrying `status`, `discountValue`, `totalUsage`, and
    `products`, and `createdAt` / `expiredAt` are ISO 8601 strings here rather
    than millisecond timestamps. That divergence is a source-side question, not
    a docs edit — detail.mdx is intentionally left untouched. The `products`
    item fields beyond `id` should be confirmed against a live response. Only
    the headless surface is documented; the mobile surface is intentionally not
    documented here. All identifiers, names, and values are synthetic. */}
...
https://api.mayar.id/hl/v2/coupons
```


```Sandbox Sandbox
https://api.mayar.club/hl/v2/coupons
```
...
<Note>
  This list is cursor paginated. Pass the `nextStartingAfter` value from a
  response back as the `startingAfter` query parameter on the next request, and
  keep looping while `hasMore` is `true`. When `hasMore` is `false`,
  `nextStartingAfter` is `null` and the whole list has been retrieved.
</Note>
