...
```Sandbox Sandbox
https://api.mayar.io/hl/v2/coupons/validate
```
...
<ResponseField name="404 Coupon not found" type="object">
  Returned with `messages: "Gagal! Kode diskon ini tidak ditemukan."` when no
  coupon with that code exists for this account.
</ResponseField>

<ResponseField name="400 Coupon not applicable to this product" type="object">
  Returned with `messages: "Gagal! Kode diskon ini tidak berlaku untuk produk yang dipilih."`
  when the coupon exists but is not bound to the `paymentLinkId` being checked.
</ResponseField>
