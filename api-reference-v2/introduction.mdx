...
This section documents the **V2** of the Mayar Headless API. V2 keeps the same authentication model as V1 but standardizes every response around a single envelope, described below. If you are coming from API V1, start with the [V1 to V2 migration guide](/api-reference-v2/migration-from-v1).
...
For testing purposes, you can use the webpage https://web.mayar.io/. You can log in or register first, then generate an API key from https://web.mayar.io/api-keys. (The same method as obtaining an API key on the production page).
