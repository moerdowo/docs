...
REPRESENTATIVE — derived from the V2 payment-request-create endpoint, not captured
from a live prod write. The `data` object returns the created payment request's id,
transactionId (null unless an email is provided), paymentLinkId, public link, amount,
status, and timestamps. Only `amount` is a required body field; every other field is
