...
| Response — current page | `page`, `pageSize`, `pageCount` (+ `total`/`totalTransaction`/`totalCustomer` on some endpoints) | `page`/`pageSize`/`pageCount` **removed**. Some list endpoints still return `total` (customers) or `totalTransaction` (transactions), but those are **all-time** totals that do **not** follow the request filters/date range, and the product list returns no `total`. For aggregated totals use [`GET /hl/v2/statistics`](/api-reference-v2/statistics/statistics). |
...
| Statistics (counts + TPV) | — | `GET /hl/v2/statistics` *(V2-only; all-time totals)* |
...
- **List totals** — `total` (customers) and `totalTransaction` (transactions) are **all-time**
  account totals; they do **not** follow the request filters or date range, and the product list
  returns no `total`. For aggregated counts (products, customers, transactions) and TPV, use
  [`GET /hl/v2/statistics`](/api-reference-v2/statistics/statistics).
...
- [ ] Stop reading `page`, `pageSize`, and `pageCount` from list responses. If you used
      `total`/`totalTransaction`/`totalCustomer`, note they are **all-time** (not filter-aware) or
      absent (product list) — use `GET /hl/v2/statistics` for aggregated totals.
