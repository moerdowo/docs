...
- **List totals** — `/hl/v2/customers` and `/hl/v2/transactions` return a `total` that is **all-time**
  (it does **not** follow the request filters or date range), and the product list returns no
  `total`. For aggregated counts (products, customers, transactions) and TPV, use
...
- [ ] Stop reading `page`, `pageSize`, and `pageCount` from list responses. If you relied on list
      totals, note they are **all-time** (not filter-aware) or absent (product list) — in V2 the
      field is `total`; use `GET /hl/v2/statistics` for aggregated totals.
