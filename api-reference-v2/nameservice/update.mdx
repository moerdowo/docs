...
{/*
  ...
  POST /hl/v2/name-services/update is not yet reachable on the public V2 gateway (calls currently
  return 404). This page documents the PLANNED V2 contract — see the not-yet-available notice below.
  The example below is derived from the V2 source surface plus synthetic values (no live capture to
  avoid mutating merchant data). Field names in `data` are source-confirmed from the stored record;
  confirm exact values/types once the route is enabled. The resource `id` is read from the REQUEST
  BODY (there is no id URL segment). The `payload`, `credential`, and `event` fields are stored as
  serialized JSON strings. Envelope key is PLURAL `messages` for this group (writes AND errors).
*/}
