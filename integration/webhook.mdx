...
![Gambar HD7 Pn](/images/gambarHD7.png)

### Add a New Webhook

You can configure Webhooks via the Mayar dashboard, here are the steps:

1. For starting using Mayar Webhook, you need account at https://mayar.id. Please use https://web.mayar.io for testing
...
To ensure that webhook payloads are genuinely from Mayar, each webhook request includes a verification token in the request header.

#### X-Callback-Token Header

Every webhook request from Mayar includes an `X-Callback-Token` header that you should validate on your server:

```
X-Callback-Token: <your_webhook_token>
```

#### How to Get Your Webhook Token

1. Log in to your Mayar dashboard at https://web.mayar.id
2. Go to **Integration** -\> **Webhook page**
3. Copy your **Webhook Token**

#### Validating Webhook Authenticity

Compare the `X-Callback-Token` header from the incoming webhook request with your stored Webhook Token. If they match, the webhook is authentic and from Mayar.
